...
group_to = format(telegram_link)+input("name of group to add to?")
...
SENT_MEMBERS_LIST = 'reports/'+format(group_name_only)+'-already_added_members.csv'
NOT_SENT_MEMBERS_LIST = 'reports/'+format(group_name_only)+'-cant_be_added_members.csv'
ALREADY_ADDED_PATH = 'reports/'+format(group_name_only)+'-sent_ids_adding.json'
...
for session in all_sessions:
    if is_all_user_sent:
        print("[+] adding members finished to all user in group. Closing script.")
        exit()
    OVERALL_COUNTER += 1
    # if (OVERALL_COUNTER) <= 62:
    #     continue
    ...
    if not is_joined_to_group.get(session):
        try:
            ...
            client(JoinChannelRequest(group_to))
            print("[+] Joined to ",group_to)
        except:
    ...
    async def main():
        ...
        async for user in client.iter_participants(group):
            ...
            if str(user.id) in sent_ids or user.is_self or user.bot :
                print('already sent to:'+format(user.id)+' '+format(user.first_name))
                if not is_new_user: 
            ...
            try:
                # print('trying adding member '+format(user.id)+'...') 
                # await client.send_message(user, message_data, file=message_file_path)
                ...
                # print("added to json")
                print(f"[+] member added {user.id}, {user.first_name}")
                ...
                # await client(LeaveChannelRequest(group_to))
                # print("[+] left from "+format(group_to))
                time.sleep(0.1)#which one is it?
                ...
                #     continue
                if 'write in this chat' in str(ex):
                    print (format(OVERALL_COUNTER)+'. '+format(datetime.now().time())+' {} is banned'.format(session))
                    # await client(LeaveChannelRequest(group))
                    # print("[+] left from "+format(group))
                    # await client(LeaveChannelRequest(group_to))
