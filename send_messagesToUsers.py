...
with open('message_content.txt', encoding="utf8") as message:
    message_data = message.read().strip()

message_file_path_gif = 'message_image.JPG'
...
# MEMBERS_LIST_FILE = 'lists/members.txt'
# MEMBERS_LIST_FILE = 'lists/all-list-29-06-21.txt'
OUTPUT_FILE = input("write down the member list name: ")
MEMBERS_LIST_FILE = 'lists/' + OUTPUT_FILE + '.txt'

MAX_USERS_IN_SESSION = int( input("write how many messages before sleeping: default 6 (number only):"))
HOURS_TO_SLEEP = int( input("write how many hours to sleep: default 24 (number only):"))

# MEMBERS_LIST_FILE = 'lists/groups_admins.txt'
ALREADY_ADDED_MEMBERS_LIST_FILE = 'lists/already_added_message.txt'
NOT_ADDEDABLE_MEMBERS_LIST_FILE = 'lists/already_added_not_good_message.txt'
...
bot_token ="1670407155:AAGdxim9j0vQIqvhNZgOnMT_QTwsb1r0D6w"
bot_token_pythonbot='1120359037:AAHNSYH9A688-gDFEcLMIQ9S2e5FQJ625Ls'
COUNTER_SKIPPED = 0
COUNTER_DEAD = 0
COUNTER_BANNED = 0
COUNTER_GOOD = 0
...
for session in all_sessions:
    ...
    try:
        #bot_token = session.replace('.session','')
        client = TelegramClient(session, APP_ID_CODE[TELEGRAM_SELECTOR], APP_ID[TELEGRAM_SELECTOR]).start(bot_token=bot_token)#(phone=bot_token)#
    ...
    with open(MEMBERS_LIST_FILE, 'r') as te:
        ...
        for t in to_be_added:
            ...
            try:
                ...
                client.send_message(t, message_data,parse_mode=ParseMode.HTML, file=message_file_path_gif)
                print(f"[+] Message sent to {t}")
                ...
                print(format(datetime.now().time())+' '+format(limit_counter)+' sent to {}'.format(t))
                # print(limit_counter);
                limit_counter += 1
                people_counter += 1
                time.sleep(0.1)
