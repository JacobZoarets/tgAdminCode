...
OUTPUT_FILE = input("write a the list name:")
# OUTPUT_FILE = 'lists/all-list-29-06-21.txt'
OUTPUT_FILE_CSV = 'lists/'+OUTPUT_FILE+'.csv'
OUTPUT_FILE_TXT = 'lists/'+OUTPUT_FILE+'.txt'

OUTPUT_FILE_groups = 'lists/good_groups.txt'

DAYS_IN_GROUP = int( input("write the maximum days back default 30 (number only):"))
...
for session in all_sessions: 
    ...
    if OVERALL_COUNTER == len(GROUP_LINK):
    ...
    with open(OUTPUT_FILE_CSV, 'a' ,encoding='utf-8') as output:
        for p in participants:
            # if p.phone or p.access_hash or p.username or p.first_name or p.last_name:
            if p.participant is not NULL and hasattr(p.participant, 'date'):
                # if type(p.status) is UserStatusOffline:
                # if type(p.status) is UserStatusOnline:
                if (datetime.datetime.utcnow() - p.participant.date.replace(tzinfo=None)).days < DAYS_IN_GROUP:
        ...
        # logging.info(p.phone)
                    ...
                    output.write(format(str(p.access_hash) + "'" )+', ')
                    ...
                    output.write(format(str(p.participant.date))+', ') 
                    if hasattr(p.status, 'was_online'):                    
                        output.write(format(str(p.status.was_online))+', ')
    ...
    with open(OUTPUT_FILE_TXT, 'a' ,encoding='utf-8') as output_txt:
        for p in participants:
            # if p.phone or p.access_hash or p.username or p.first_name or p.last_name:
            if p.participant is not NULL and hasattr(p.participant, 'date'):
                # if type(p.status) is UserStatusOffline:
        ...
        # logging.info(p.phone)
                    if(p.username):
                        print(format(p.username))
                        output_txt.write(format(p.username))
                        output_txt.write('\n')
